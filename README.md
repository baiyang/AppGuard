...
FastAPI 项目安装产品专用运行时 wheel 时，添加 `[fastapi]` 以安装适配器依赖；将下方文件名替换为 `appguard build-runtime` 实际生成的 wheel 路径：

```sh
python3.11 -m pip install '/path/to/appguard_product_runtime-0.0.1-cp311-cp311-linux_x86_64.whl[fastapi]'
```
