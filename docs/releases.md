...
4. 使用 Flask 示例固定的已发布版本 `0.0.2` 构建 Docker 镜像，审计各层，并验证授权与业务接口。待发布版本的运行时同时由源码测试和包安装检查覆盖。
...
每次正式 PyPI 发布并验证成功后，将示例 Dockerfile、安装命令和文档中的版本同步到最新已发布版本，通过 CI 验证示例交付流程后合入 `main`。
